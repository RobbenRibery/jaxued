...
class AdversaryActorCritic(nn.Module):
    ...
    def __call__(self, inputs: Tuple[Observation, chex.Array], hidden):
        ...
        img_embed = nn.Conv(128, kernel_size=(3, 3), strides=(1, 1), padding="VALID")(
            obs.image
        )
        img_embed = img_embed.reshape(*img_embed.shape[:-3], -1)
        img_embed = nn.relu(img_embed)
...
def main(config=None, project="JAXUED_TEST"):
    ...
    def log_eval(stats):
        ...
        def make_caption(i):
            pro_mean_returns = jnp.round(
                stats["pro_mean_returns"][-1][i], 2
            )  # .flatten()
            ant_max_returns = jnp.round(
                stats["ant_max_returns"][-1][i], 2
            )  # .flatten()
        ...
        log_dict.update(
            {
                f"images/levels": [
                    wandb.Image(np.array(image), caption=make_caption(i))
                    for i, image in enumerate(stats["levels"])
                ]
            }
        )
    ...
    adv_env = MazeEditor(
        env,
        random_z_dimensions=config["adv_random_z_dimension"],
        zero_out_random_z=config["adv_zero_out_random_z"],
    )
    eval_env = env
    ...
    def create_train_state(rng):
        def create_inner_train_state(
            rng, env, env_params, network_cls, prefix, network_kws={}
        ):
            ...
            obs = jax.tree_map(
                lambda x: jnp.repeat(
                    jnp.repeat(x[None, ...], config["num_train_envs"], axis=0)[
                        None, ...
                    ],
                    256,
                    axis=0,
            ...
            network_params = network.init(
                rng, init_x, network_cls.initialize_carry((config["num_train_envs"],))
            )
            tx = optax.chain(
        ...
        return TrainState(
            update_count=0,
            pro_train_state=create_inner_train_state(
                rng_pro, env, env_params, ActorCritic, "student_"
            ),
            ant_train_state=create_inner_train_state(
                rng_ant, env, env_params, ActorCritic, "student_"
            ),
            adv_train_state=create_inner_train_state(
                rng_adv,
                adv_env,
                adv_env_params,
                AdversaryActorCritic,
                "adv_",
                network_kws={"max_timesteps": config["adv_num_steps"]},
            ),
        )

    def train_step(carry, _):
        def rollout(
            rng, env, env_params, train_state, init_hstate, levels, num_steps, prefix
        ):
            # Single rollout
            rng, _rng = jax.random.split(rng)
            init_obs, init_env_state = jax.vmap(
                env.reset_to_level, in_axes=(0, 0, None)
            )(jax.random.split(_rng, config["num_train_envs"]), levels, env_params)
            (
            ...
            advantages, targets = compute_gae(
                config[f"{prefix}gamma"],
                config[f"{prefix}gae_lambda"],
                last_value,
                values,
                rewards,
                dones,
            )
            return (obs, actions, dones, log_probs, values, targets, advantages), (
                dones,
                rewards,
                last_env_state,
                last_value,
        ...
        rng, _rng = jax.random.split(rng)
        empty_levels = jax.tree_map(
            lambda x: jnp.array([x]).repeat(config["num_train_envs"], axis=0),
            sample_empty_level(),
        )
        adv_rollout, (_, _, last_env_state, last_value, _) = rollout(
            _rng,
            adv_env,
            adv_env_params,
            adv_train_state,
        ...
        # protagonist rollout
        rng, _rng = jax.random.split(rng)
        pro_rollout, (dones, rewards, _, _, _) = rollout(
            _rng,
            env,
            env_params,
            pro_train_state,
        ...
        # antagonist rollout
        rng, _rng = jax.random.split(rng)
        ant_rollout, (dones, rewards, _, _, _) = rollout(
            _rng,
            env,
            env_params,
            ant_train_state,
        ...
        (rng, pro_train_state), pro_losses = update(
            rng,
            pro_train_state,
            ActorCritic.initialize_carry((config["num_train_envs"],)),
            pro_rollout,
        ...
        (rng, ant_train_state), ant_losses = update(
            rng,
            ant_train_state,
            ActorCritic.initialize_carry((config["num_train_envs"],)),
            ant_rollout,
    ...
    def train_and_eval_step(runner_state, _):
        ...
        metrics["levels"] = jax.vmap(env_renderer.render_level, (0, None))(
            jax.tree_map(lambda x: x[-1], metrics["levels"]), env_params
        )
    ...
    def eval_checkpoint(og_config):
        ...
        def load(rng_init, checkpoint_directory: str):
            ...
            train_state = train_state_og.replace(
                pro_train_state=train_state_og.pro_train_state.replace(params=params)
            )
...
if __name__ == "__main__":
    ...
    group.add_argument(
        "--adv_zero_out_random_z", action=argparse.BooleanOptionalAction, default=False
    )
    ...
    if config["num_env_steps"] is not None:
        config["num_updates"] = config["num_env_steps"] // (
            2 * config["num_train_envs"] * config["student_num_steps"]
        )
    config["group_name"] = "paired4_" + "".join(
        [
            str(config[key])
