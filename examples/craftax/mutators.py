...
def seq_edits(
    ...
    state: EnvState,
    n: int = 1,
    mutators_collection: List[Callable] = None,
    ) -> EnvState:
    ...
    _, final_env_state = final_carry
    return final_env_state


def make_mutator_craftax_swap(
    static_env_params: StaticEnvParams, only_middle=True, is_craftax_classic=False
):
    ...
    def add_blocktype(rng, level: EnvState, blocktype: BlockType) -> EnvState:
        ...
        def do_swap(level):
            ...
            level = level.replace(
                map=level.map.at[0, row_old, col_old].set(tile_i_am_going_to_delete)
            )
        ...
        level = jax.lax.cond(does_new_block_exist_anywhere, do_swap, dont_swap, level)
    ...
    def mutate_level(rng, level: EnvState, n=1):
        if is_craftax_classic:
            level = level.replace(map=level.map[None])

        def _single_mutate(carry, _):
            rng, level = carry
            rng, _rng, _rng2 = jax.random.split(rng, 3)
            block_to_add = good_blocks_to_choose_from[
                jax.random.randint(_rng, (), 0, NUM_BLOCKS_TO_CHOOSE)
            ]
        ...
        if is_craftax_classic:
            level = level.replace(map=level.map[0])
        return level
...
def make_mutator_craftax_swap_restricted(
    static_env_params: StaticEnvParams,
    one_should_be_middle=False,
    is_craftax_classic=False,
):
    ...
    def single_step(rng, level: EnvState):
        ...
        new_block = jax.lax.select(
            ...
            jax.random.choice(_rng2, for_stone),
            new_block,
        )
        new_block = jax.lax.select(
            block_to_add == BlockType.COAL.value,
            jax.random.choice(_rng3, for_coal),
            new_block,
        )
        new_block = jax.lax.select(
            block_to_add == BlockType.DIAMOND.value,
            jax.random.choice(_rng4, for_diamond),
            new_block,
        )
        new_block = jax.lax.select(
            block_to_add == BlockType.IRON.value,
            jax.random.choice(_rng5, for_iron),
            new_block,
        )
...
def make_mutator_craftax_mutate_angles(
    generate_world, static_env_params: StaticEnvParams, params_to_use
):
    ...
    def mutate_level(rng, level: EnvState, n=1):
        ...
        new_angles = jax.tree_map(
            lambda x, rr: jnp.clip(
                x + jax.random.uniform(rr, x.shape, minval=-0.2, maxval=0.2), 0, 1
            ),
        ...
        return generate_world(
            rng,
            params_to_use.replace(fractal_noise_angles=new_angles),
            static_env_params,
