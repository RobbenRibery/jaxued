...
import wandb
from jaxued.environments.gymnax.pendulum import (
    Pendulum,
    make_eval_levels_and_names as eval_levels_pendulum,
    make_level_generator as make_level_generator_pendulum,
)
...
from jaxued.environments.gymnax.cartpole import (
    CartPole,
    make_eval_levels_and_names as eval_levels_cp,
    make_level_generator as make_level_generator_cp,
)
from jaxued.environments.gymnax.acrobot import (
    Acrobot,
    make_eval_levels_and_names as eval_levels_acrobot,
    make_level_generator as make_level_generator_acrobot,
)
...
class ActorCriticRNN(nn.Module):
    ...
    def __call__(self, x, hidden):
        ...
        hidden, embedding = ResetRNN(nn.GRUCell(features=256))(
    ...
    def initialize_carry(batch_dims):
        ...
        return cell.initialize_carry(jax.random.PRNGKey(0), (*batch_dims, 256))
...
def main(config=None, project="JAXUED_TEST"):
        ...
        log_dict.update(
            {
                ...
                for name, solve_rate in zip(EVAL_LEVEL_NAMES, solve_rates)
            }
        )
        log_dict.update({"solve_rate/mean": solve_rates.mean()})
        log_dict.update(
            {f"return/{name}": ret for name, ret in zip(EVAL_LEVEL_NAMES, returns)}
        )
    ...
    for eval_step in range(config["num_updates"] // config["eval_freq"]):
        ...
        runner_state, metrics = vmap_train_eval_step(
            (jax.random.split(runner_state[0], NUM_REPEATS), runner_state[1]), None
        )
        ...
        log_eval(
            ...
            train_state_to_log_dict(
                jax.tree_util.tree_map(lambda x: x[0], runner_state[1]), level_sampler
            ),
        )
        if config["checkpoint_save_interval"] > 0:
...
if __name__ == "__main__":
    ...
    group.add_argument(
        "--num_repeats", type=int, default=10
    )  # how many parallel seeds to run at once.
