"""Maze level representation and built-in evaluation layouts."""
...
@struct.dataclass
class Level:
    """Static parameters that define a maze instance.

    Attributes:
        wall_map: Boolean grid whose true entries are walls.
        goal_pos: Integer ``(x, y)`` goal position.
        agent_pos: Integer ``(x, y)`` starting position.
        agent_dir: Starting direction index in ``[0, 3]``.
        width: Unpadded level width.
        height: Unpadded level height.
    """
    ...
    def is_well_formatted(self):
        """Validate structural invariants of the level.

        Returns:
            JAX boolean scalar indicating whether positions, walls, bounds, and
            direction satisfy the existing level constraints.
        """
    ...
    def from_str(cls, level_str):
        ...
        ``#`` denotes a wall, ``.`` an empty cell, ``G`` a goal, and one of
        ``>``, ``v``, ``<``, or ``^`` denotes the agent and its orientation.

        Args:
            level_str: Rectangular ASCII representation of a maze.

        Returns:
            Parsed maze level.

        Raises:
            AssertionError: If rows have inconsistent lengths, the agent or goal
                is missing, or the agent occurs more than once.
            Exception: If an unsupported character occurs.
        """
        ...
        return Level(
            jnp.array(wall_map),
            *map(lambda x: jnp.array(x, dtype=jnp.uint32), (goal_pos[0], agent_pos)),
            jnp.array(agent_dir, dtype=jnp.uint8),
            ncols,
            nrows,
        )

    def to_str(self):
        """Serialize the level to its ASCII representation.

        Returns:
            Rectangular string using the symbols accepted by :meth:`from_str`.
        """
    ...
    def pad_to_shape(self, max_width, max_height):
        """Pad the wall map to fixed dimensions with walls.

        Args:
            max_width: Requested padded width.
            max_height: Requested padded height.

        Returns:
            A level copy whose wall map has the requested final dimensions.

        Raises:
            AssertionError: If either requested dimension is smaller than the
                current wall map.
        """
    ...
    def stack(cls, levels):
        """Pad and stack differently sized levels into one PyTree batch.

        Args:
            levels: Non-empty sequence of :class:`Level` instances.

        Returns:
            Batched level PyTree padded to the maximum width and height in the
            input sequence.
        """
        level_dims = np.array(
    ...
    def load_prefabs(cls, ids):
        """Load and batch named built-in levels.

        Args:
            ids: Sequence of keys in :data:`prefabs`.

        Returns:
            Batched levels padded to a common shape.

        Raises:
            KeyError: If an identifier is absent from :data:`prefabs`.
        """
