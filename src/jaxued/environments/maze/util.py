"""Procedural generators and mutation operators for maze levels."""
...
def make_level_generator(
    height: int, width: int, n_walls: int
) -> Callable[[chex.PRNGKey], Level]:
    """Create a random maze-level generator.

    Args:
        height: Height of generated wall maps.
        width: Width of generated wall maps.
        n_walls: Number of wall-placement samples made per level.

    Returns:
        Pure callable that maps a JAX random key to a :class:`Level`.
    """

    def sample(rng: chex.PRNGKey) -> Level:
        """Sample one maze level.

        Args:
            rng: JAX random key used for walls, positions, and direction.

        Returns:
            Random level with distinct unoccupied agent and goal positions.
        """
        ...
        choices = jax.random.choice(
            rng_wall, max_w * max_h, shape=(max_w * max_h,), p=valid_mask, replace=True
        )
        choices = jnp.where(all_pos < n_walls, choices, choices[0])
        occupied_mask = (
            jnp.zeros(max_w * max_h, dtype=jnp.bool_).at[choices].set(n_walls > 0)
            | ~valid_mask
        )
        ...
        agent_idx = jax.random.choice(
            rng_agent_pos, all_pos, shape=(1,), p=(~occupied_mask).astype(jnp.float32)
        )
        occupied_mask = occupied_mask.at[agent_idx].set(True)
        agent_pos = jnp.array(
            [agent_idx % max_w, agent_idx // max_w], dtype=jnp.uint32
        ).flatten()

        # Reset agent direction
        agent_dir = jax.random.choice(
            rng_agent_dir, jnp.arange(len(DIR_TO_VEC), dtype=jnp.uint8)
        )

        # Reset goal position
        goal_idx = jax.random.choice(
            rng_goal, all_pos, shape=(1,), p=(~occupied_mask).astype(jnp.float32)
        )
        goal_pos = jnp.array(
            [goal_idx % max_w, goal_idx // max_w], dtype=jnp.uint32
        ).flatten()
...
def make_level_mutator(
    ...
    """Create the original general-purpose random level mutator.

    Each edit independently adds a wall, removes a wall, moves the agent, or
    moves the goal. Displaced agent and goal positions are repaired after all
    edits.

    Args:
        max_num_edits: Static upper bound used to size the JAX scan.

    Returns:
        Callable ``mutate(rng, level, num_edits)`` that returns a mutated level.
    """

    def mutate(rng: chex.PRNGKey, level: Level, num_edits: int = 1) -> Level:
        """Apply a bounded number of random edits.

        Args:
            rng: JAX random key used to choose edit types and locations.
            level: Source level.
            num_edits: Number of active edits, capped structurally by
                ``max_num_edits``.

        Returns:
            Mutated level with valid, distinct agent and goal positions.
        """
        max_w, max_h = level.wall_map.shape[1], level.wall_map.shape[0]
        all_pos = jnp.arange(max_w * max_h, dtype=jnp.uint32)
        valid_mask = (all_pos % max_w < level.width) & (all_pos < max_w * level.height)

        rng, rng_perm, rng_loc, rng_action, rng_goal, rng_agent, rng_agent_dir = (
            jax.random.split(rng, 7)
        )
        edit_locs = jax.random.permutation(
            rng_perm,
            jnp.unique(
                jax.random.choice(
                    rng_loc, all_pos, shape=(max_num_edits,), p=valid_mask
                ),
                size=max_num_edits,
                fill_value=-1,
            ),
        ...
        edit_locs = jnp.where(
            jnp.cumsum(edit_locs != -1) <= num_edits, edit_locs, -1
        )  # mask out extra action
        ...
        def mutation_step(carry, input):
            ...
            def on_mutate(
                rng, level, edit_loc, action, agent_displaced, goal_displaced
            ):
                x, y = edit_loc % max_w, edit_loc // max_w

                agent_displaced = (
                    (level.agent_pos[0] == x) & (level.agent_pos[1] == y)
                ) | agent_displaced
                goal_displaced = (
                    (level.goal_pos[0] == x) & (level.goal_pos[1] == y)
                ) | goal_displaced

                def add_wall(rng):
                    return (
                        rng,
                        level.replace(wall_map=level.wall_map.at[y, x].set(True)),
                        agent_displaced,
                        goal_displaced,
                    )
                ...
                def remove_wall(rng):
                    return (
                        rng,
                        level.replace(wall_map=level.wall_map.at[y, x].set(False)),
                        agent_displaced,
                        goal_displaced,
                    )
                ...
                def move_agent_pos(rng):
                    ...
                    agent_dir = jnp.array(
                        jax.random.choice(rng_dir, 4), dtype=jnp.uint8
                    )
                    return (
                        rng,
                        level.replace(
                ...
                return jax.lax.switch(
                    action, [add_wall, remove_wall, move_agent_pos, move_goal_pos], rng
                )
        ...
        (rng, level, agent_displaced, goal_displaced), _ = jax.lax.scan(
            mutation_step, (rng, level, False, False), (edit_locs, actions)
        )
        ...
        p = (
            (~level.wall_map.flatten() & valid_mask)
            .at[agent_idx]
            .set(
                agent_displaced
                & ~(level.wall_map[level.agent_pos[1], level.agent_pos[0]])
            )
        )
        ...
        new_level = level.replace(
            goal_pos=goal_pos, agent_pos=agent_pos, agent_dir=agent_dir
        )
...
def make_level_mutator_minimax(
    ...
    """Create a Minimax-compatible wall/goal mutator.

    The returned mutator samples no-op, wall-flip, and goal-move operations.
    This function preserves the mutation behavior adapted from Minimax.

    Args:
        max_num_edits: Static upper bound used to size sampled mutations.

    Returns:
        Callable ``mutate(rng, level, n)`` that applies the first ``n`` sampled
        operations.
    """

    class Mutations(IntEnum):
        """Mutation identifiers sampled by the Minimax-compatible mutator."""
    ...
    def move_goal_flip_walls(rng, level, n=1):
        """Apply sampled goal moves and wall flips.

        Args:
            rng: JAX random key used to choose mutations and locations.
            level: Source maze level.
            n: Number of active operations.
        ...
        def _mutate(carry, step):
            ...
            def _apply(rng, state):
                ...
                next_state = jax.tree_util.tree_map(
                    lambda x, y: jax.lax.select(is_flip_wall, x, y),
                    mutated_state,
                    state,
                )
                ...
                next_state = jax.tree_util.tree_map(
                    lambda x, y: jax.lax.select(is_move_goal, x, y),
                    mutated_state,
                    next_state,
                )
            ...
            return jax.lax.cond(
                mutation != -1, _apply, lambda *_: state, rng, state
            ), None
