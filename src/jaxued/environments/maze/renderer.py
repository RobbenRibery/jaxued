"""JIT-compatible pixel renderer for maze levels and states."""
...
class MazeRenderer(object):
    """Render maze grids and the agent's visible region.

    Args:
        env: Maze whose dimensions and view size determine rendering.
        tile_size: Width and height in pixels of each grid tile.
        render_border: Whether to add a one-tile wall border.
    """

    def __init__(self, env: Maze, tile_size: int = 32, render_border: bool = True):
        """Initialize a renderer and precompute its tile atlas.

        Args:
            env: Maze associated with rendered states.
            tile_size: Pixel dimensions of each square tile.
            render_border: Whether rendered grids include an outer wall.
        """
        self.env = env
    ...
    def render_level(self, level, env_params):
        """Render a level-like record using the state renderer.

        Maze levels and states expose the position and wall fields needed for
        rendering, so no environment reset is required.

        Args:
            level: Level or state record containing agent, goal, and wall data.
            env_params: Runtime environment parameters retained for renderer
                interface consistency.

        Returns:
            RGB image as an unsigned integer JAX array.
        """
    ...
    def render_state(self, env_state: EnvState, env_params: EnvParams) -> chex.Array:
        """Render a maze state and highlight the agent's view window.

        Args:
            env_state: State containing agent, goal, and wall locations.
            env_params: Runtime environment parameters retained for renderer
                interface consistency.

        Returns:
            RGB image with shape ``(pixel_height, pixel_width, 3)``.
        """
...
def _make_tile_atlas(tile_size):
    """Create raster tiles for empty, wall, goal, and agent cells.

    Args:
        tile_size: Width and height of each square tile in pixels.

    Returns:
        NumPy array with shape ``(7, tile_size, tile_size, 3)``.
    """
    TRI_COORDS = np.array(
        [
            [0.12, 0.19],
