...
class CraftaxUEDWrapper(UnderspecifiedEnv):
    """Adapt upstream Craftax environments to JaxUED's reset-to-level API."""
    ...
    def reset_env_to_level(
        ...
        del rng, params
        return self.get_obs(level), level
...
class CraftaxLogWrapper(UnderspecifiedEnv):
    """Log episode returns, lengths, achievements, and current floor."""
    ...
    def reset_env_to_level(
        ...
        del rng, params
        state = CraftaxLogEnvState(level, 0.0, 0, 0.0, 0, 0)
    ...
    def get_obs(self, state: CraftaxLogEnvState) -> chex.Array:
