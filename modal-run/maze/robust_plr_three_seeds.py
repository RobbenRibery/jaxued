"""Launch the same Maze Robust PLR baseline for three seeds on Modal.
...
app = modal.App("jaxued-maze-robust-plr-three-seeds", image=image)
...
def run_robust_plr_seed(request: SeedRequest) -> tuple[int, str]:
    """Run one seed through the standard Robust PLR training path."""
    ...
    checkpoint_path = run_training(build_robust_plr_command(run), run)
    return seed, checkpoint_path


@app.local_entrypoint()
def main(
    run_name: str = "maze_robust_plr_three_seeds",
    ...
    results = list(run_robust_plr_seed.map(requests, return_exceptions=True))
    ...
    if failures:
        ...
        raise RuntimeError(f"Robust PLR failed for seed(s): {failed_seeds}")
