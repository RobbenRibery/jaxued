"""Launch mean-absolute-advantage Maze PLR for three seeds on Modal.
...
app = modal.App("jaxued-maze-mean-absolute-advantage-three-seeds", image=image)
...
def run_mean_absolute_advantage_seed(request: SeedRequest) -> tuple[int, str]:
    """Run one seed through Robust PLR with mean-absolute-advantage scoring."""
    ...
    checkpoint_path = run_training(
        build_mean_absolute_advantage_plr_command(run),
        run,
    )
...
def main(
    run_name: str = "maze_mean_absolute_advantage_three_seeds",
    ...
    results = list(
        run_mean_absolute_advantage_seed.map(requests, return_exceptions=True)
    )
    ...
    if failures:
        ...
        raise RuntimeError(
            f"Mean-absolute-advantage PLR failed for seed(s): {failed_seeds}"
        )
