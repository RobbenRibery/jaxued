...
#
# Usage:
#   SEED=3 N_WALLS=60 SIN_N_VIRTUAL_UPDATES=4 SIN_NUM_ROLLOUTS_PER_LEVEL=8 SIN_SCORE_BATCH_SIZE=2 \
...
SIN_SCORE_BATCH_SIZE="${SIN_SCORE_BATCH_SIZE:-2}"
...
if ! [[ "${SIN_SCORE_BATCH_SIZE}" =~ ^[0-9]+$ ]] || [[ "${SIN_SCORE_BATCH_SIZE}" -lt 1 ]]; then
    echo "Error: sin_score_batch_size must be a positive integer, got '${SIN_SCORE_BATCH_SIZE}'." >&2
    exit 1
fi
PROJECT_NAME="ued"
SCORE_FUNCTION="s_in"
METHOD_NAME="robust_plr"
WANDB_EXPERIMENT_NAME="${METHOD_NAME}-s_in-seed${SEED}-walls${N_WALLS}-v${SIN_N_VIRTUAL_UPDATES}-r${SIN_NUM_ROLLOUTS_PER_LEVEL}-b${SIN_SCORE_BATCH_SIZE}-ppoloss"
...
uv run -m examples.maze_plr \
    ...
    --sin_score_batch_size "$SIN_SCORE_BATCH_SIZE" \
